...
public class BusLineEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.busline_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑公交线路信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_name.getText().toString().equals("")) {
						Toast.makeText(BusLineEditActivity.this, "线路名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_startTime.getText().toString().equals("")) {
						Toast.makeText(BusLineEditActivity.this, "首班车时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_endTime.getText().toString().equals("")) {
						Toast.makeText(BusLineEditActivity.this, "末班车时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_company.getText().toString().equals("")) {
						Toast.makeText(BusLineEditActivity.this, "所属公司输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_tjzd.getText().toString().equals("")) {
						Toast.makeText(BusLineEditActivity.this, "途径站点输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_polylinePoints.getText().toString().equals("")) {
						Toast.makeText(BusLineEditActivity.this, "地图线路坐标输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传公交线路信息*/
					BusLineEditActivity.this.setTitle("正在更新公交线路信息，稍等...");
					String result = busLineService.UpdateBusLine(busLine);
	...
	private void initViewData() {
		...
		this.TV_lineId.setText(lineId+"");
		this.ET_name.setText(busLine.getName());
		for (int i = 0; i < busStationList.size(); i++) {
			if (busLine.getStartStation() == busStationList.get(i).getStationId()) {
				this.spinner_startStation.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < busStationList.size(); i++) {
			if (busLine.getEndStation() == busStationList.get(i).getStationId()) {
				this.spinner_endStation.setSelection(i);
				break;
			}
		}
		this.ET_startTime.setText(busLine.getStartTime());
		this.ET_endTime.setText(busLine.getEndTime());
		this.ET_company.setText(busLine.getCompany());
		this.ET_tjzd.setText(busLine.getTjzd());
		this.ET_polylinePoints.setText(busLine.getPolylinePoints());
	}
	@Override
